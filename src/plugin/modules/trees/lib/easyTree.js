define([
    ...
    function EasyTree(canvasId, treeString, nodeIdToNameMap, leafClickListener, nodeColorFunc) {
        ...
        var kn_g_tree = knhx.kn_parse(treeString);

        if (nodeIdToNameMap) {
            var nodePos, nodeId, nodeName;
            for (nodePos in kn_g_tree.node) {
                if (kn_g_tree.node.hasOwnProperty(nodePos)) {
                    nodeId = kn_g_tree.node[nodePos].name;
                    nodeName = nodeIdToNameMap[nodeId];
                    if (nodeName) {
        ...
        // Oops, easytree takes over for knhx_init
        // knhx.knhx_init(canvasId, 'text_'+canvasId, kn_g_conf, kn_g_tree);
        ...
        function plot(canvas, kn_g_tree, kn_g_conf) {
            knhx.kn_plot_core(canvas, kn_g_tree, kn_g_conf);
            ...
            ctx.drawText(kn_g_conf.font, kn_g_conf.fontsize, x, y + kn_g_conf.fontsize * 0.8 + kn_g_conf.fontsize / 3, text);
            changeLayoutX = x;
            changeLayoutY = y;
            changeLayoutW = w;
            changeLayoutH = h;
        ...
        function changeLayout(isCircular) {
            ...
            knhx.kn_count_tips(kn_g_tree);
            kn_g_conf.is_real = knhx.kn_calxy(kn_g_tree, kn_g_conf.is_real);
        ...
        function ev_canvas(ev) {
            ...
            if (kn_g_tree) {
                var id = knhx.kn_get_node(kn_g_tree, kn_g_conf, ev._x, ev._y);
                if (id >= 0 && id < kn_g_tree.node.length) {
                    ...
                    if (i < tree.node.length && tree.node[i].child.length) {
                        ...
                        kn_count_tips(tree);
                        conf.is_real = knhx.kn_calxy(tree, conf.is_real);
                        kn_g_tree = tree;
                        kn_g_conf = conf;
        ...
        if (tree.error) {
            if (tree.error & 1) {
                window.alert("Parsing ERROR: missing left parenthesis!");
            } else if (tree.error & 2) {
                window.alert("Parsing ERROR: missing right parenthesis!");
            } else if (tree.error & 4) {
                window.alert("Parsing ERROR: missing brackets!");
            } else {
                window.alert("Unknown parsing ERROR: " + tree.error);
            }
        } else {
            conf.is_real = knhx.kn_calxy(tree, conf.is_real);
